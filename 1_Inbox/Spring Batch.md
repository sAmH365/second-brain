...
* Job의 구성요소:
	...
	* [[#Step (단계)]]
* Job을 빌드하는 방법:
	* **JobBuilder 사용**: `{java} new JobBuilder("작업명", jobRepository)`를 통해 직접 객체 생성 (`JobBuilderFactory`는 폐지됨 *(SpringBatch5.0 이후)* )
    * **Step 연결**: 생성된 빌더에 `{java}.start(stepA).next(stepB).build()` 형태로 단계를 순차적으로 연결하여 구성
### Step (단계)
...
* 동일한 jobName + 동일한 JobParameter = <span style="color:rgb(101, 147, 52)"><b>JobInstance</b></span>
### JobExecution / StepExecution (실행 상태)
* 정의: Job과 Step이 실제로 실행된 '시도(Attempt)와 결과 상태를 담은 객체'
### JobParameters (작업 매개변수)
* 정의: 배치 실행할 때 외부에서 주입하는 파라미터 (타입: `{java}String`, `{java}Long`, `{java}Double`, `{java}Date`)
* <span style="color:rgb(219, 164, 0)"><b>JobInstance를 구분하는 기준이 됨</b></span>
### JobRepository (저장소)
* 정의: 배치가 실행되는 모든 메타데이터 (Job 상태, 성공 여부, 실행 시간 등)를 DB에 저장하고 관리하는 관리자역할
* 키워드: `{java}BATCH_JOB_INSTANCE`, `{java}BATCH_JOB_EXECUTION`
...
* **ItemProssor**
	* 형태: `{java}O process(I item) throws Exception`
	* 생략 가능 (필수x)
	* 데이터 가공
...
* **ItemWriter**
	* 형태: `{java}void write(Chunk<? extends T> chunk) throws Exception;`
